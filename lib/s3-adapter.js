...
module.exports = CoreObject.extend({
  ...
  activate: function(revision) {
    var revisionWithPrefix = this.config.prefix ? this.config.prefix.concat(revision) : revision;
  ...
  _printBucketRevisions: function(revisions) {
    ...
    var revisionsList = revisions.reduce(function(prev, current, index) {
      return prev + '\n\n' + (index + 1) + ') ' + current.replace(this.config.prefix || '', '');
    }.bind(this), '');
  ...
  _removeCurrentRevisionFromContents: function(data) {
    for (var i = 0, len = data.Contents.length; i < len; i++) {
      var fileName = data.Contents[i].Key.replace(this.config.prefix || '', '');
  ...
  */
  ...
  _getUploadParams: function(key, value) {
    var params = {
      Bucket: this.config.bucket,
      Key: (this.config.prefix || '') + key + '.html',
      Body: value,
